...
description: 當使用者的需求是新增多個功能，或判斷使用者的需求適合拆分成多個 feature branch 並行開發時，或是提到 "worktree"、"git worktree"、"多分支開發"、"parallel branches"，自動觸發此 Skill。先分析需求並建議 worktree 拆分方案，經使用者確認後執行建立。
---
...
#### 2a. 專案全局上下文分析 (Context Analysis)

在草擬 Spec 之前，**強制**先執行 `list_dir` / `view_file` 掃描專案的核心檔案（例如 `package.json`、代表性的 UI 組件、重點資料檔），來確立以下資訊：
- **業務領域 (Business Domain)**: 這個專案真實的用途與上下文是什麼？（不要僅根據目錄名稱隨意猜測，需從程式碼及常數提取）。
- **設計系統 (Design System)**: 專案已經有哪些排版 Wrapper（例如 `.container`、`.section-header` 等）？新設計的 UI 必須硬性規定複用這些既有的 CSS class 或結構，保持全局視覺一致。
...
| **介面語系** | 根據專案現狀，強制指定 UI 文案所使用的語系（例如繁體中文），避免生成英文佔位符 |
| **實作避坑 (Gotchas)** | 提前預測本需求常見的 Web/React 陷阱（例如 `button` 未加 `type=button` 導致刷新、事件截斷等），預先提醒接手的 AI |
...
- 不得引入新的 npm 依賴（視情況填寫）
- 需相容既有的設計系統（具體指明應複用的 ClassName 或結構）

## 介面語系與文案

強制規定本 Feature 的所有外部展示文字、提示語等，必須使用：**[填入語系，如繁體中文]**，絕不可使用英文或空白佔位。

## 實作避坑 (Gotchas)

- （範例）確保所有內部切換按鈕加入 `type="button"` 與 `e.preventDefault()` 以避免意外觸發重新整理。
- （請補入其他前置預判的問題）
